...
async function readPrivateMessagesModule(fileName: string): Promise<string> {
  ...
  const source = readFile(cacheKey, "utf8").then((payload) => {
    // Legacy/raw payloads are accepted only to keep generated fixtures from
    // becoming a loader compatibility boundary. Published compact catalogs
    // contain deterministic raw-deflate Base64, which removes the otherwise
    // duplicated renderer source from the cache footprint.
